...
test("startup migration canonicalizes legacy Copilot and disables untouched Gemini", () => {
  const dir = mkdtempSync(join(tmpdir(), "orkworks-settings-"));
  try {
    writeFileSync(
      settingsPath(dir),
    ...
    assert.equal(settings.providers.providers.find((provider) => provider.id === "gemini")?.enabled, false);
...
test("startup migration preserves a user-modified Gemini entry", () => {
  const dir = mkdtempSync(join(tmpdir(), "orkworks-settings-"));
  try {
    ...
    assert.equal(settings.providers.providers.find((provider) => provider.id === "gemini")?.enabled, true);
    assert.equal(readFileSync(settingsPath(dir), "utf8"), raw);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test("settings memory normalizes malformed provider payloads", () => {
  ...
  try {
    ...
    assert.deepEqual(settings.providers.providers.map((entry) => entry.id), ["claude-code", "opencode", "codex", "gemini", "aider", "copilot", "ollama"]);
...
test("settings memory preserves provider revisions and canonical fallback order on write", () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      [["codex", 0], ["opencode", 1], ["gemini", 2], ["aider", 3], ["copilot", 4], ["ollama", 5], ["claude-code", 6]],
    );
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
