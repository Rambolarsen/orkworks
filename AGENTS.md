...
- (design, not yet implemented — see issue #313) `~/.orkworks/workspaces/<hash>/workflow-observations/<session-id>.ndjson` and `~/.orkworks/workspaces/<hash>/workflow-observations/sequence` — planned to hold bounded (1,000 records/2 MiB per session), sequenced, immutable `WorkflowObservation` evidence recorded through one shared module from an authenticated agent-report route and Peon inference; durable improvement evidence for Taskmaster, deliberately separate from the current-summary snapshot above (ADR 0042)
...
- Taskmaster consumes normalized metadata and proposes cross-session transitions; v1 requires explicit user approval for every action. (design, not yet implemented — see issue #313) A passive `improve_workflow` recommendation is planned as the one exception to the approval-before-action framing above only in that it would require no approval to *display* — it still could not start a session, focus a terminal, or edit a file; it could only be dismissed (ADR 0042).
