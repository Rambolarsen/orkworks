...
- `.github/workflows/pr-review.yml` posts one automated first-pass review comment on newly opened PRs that touch `apps/desktop/` or `crates/orkworksd/` **and exceed the review gate's escalation threshold** (roughly more than 8 non-docs code files or 500 non-docs changed lines), reusing the same `CLAUDE_CODE_OAUTH_TOKEN` secret. Smaller PRs are deliberately skipped — they are covered by the manual gate, Copilot review, and CI — because a full automated pass on every small PR costs more tokens than its findings justify. It is informational only — excluded from required status checks, cannot block a merge, and does not replace the manual `/code-review` review gate below; it always says so in its own comment.
