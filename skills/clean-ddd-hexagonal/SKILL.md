...
L-- Implements a port                      -> adapter (infrastructure)
```
...
L-- "Does this have the same value?"  -> Value Object (structural equality)
```
...
L-- >10 entities in aggregate                    -> Split it
```
...
|   |   L-- services            # Domain services (stateless logic)
|   L-- shared/
|       L-- errors              # Domain errors
+-- application/               # Use cases / Application services
|   +-- {use-case}/
|   |   +-- command             # Command/Query DTOs
|   |   +-- handler             # Use case implementation
|   |   L-- port                # Driver port interface
|   L-- shared/
|       L-- unit_of_work        # Transaction abstraction
+-- infrastructure/            # Adapters (external concerns)
|   +-- persistence/           # Database adapters
|   +-- messaging/             # Message broker adapters
|   +-- http/                  # REST/GraphQL adapters (DRIVER)
|   L-- config/
|       L-- di                  # Dependency injection / composition root
L-- main                        # Bootstrap / entry point
```
...
Full reference docs available from the upstream repo at https://github.com/ccheney/robust-skills/tree/main/skills/clean-ddd-hexagonal/references

| File | Purpose |
|------|---------|
| LAYERS.md | Complete layer specifications |
| DDD-STRATEGIC.md | Bounded contexts, context mapping |
| DDD-TACTICAL.md | Entities, value objects, aggregates (pseudocode) |
| HEXAGONAL.md | Ports, adapters, naming |
| CQRS-EVENTS.md | Command/query separation, events |
| TESTING.md | Unit, integration, architecture tests |
...
- [Explicit Architecture](https://herbertograca.com/2017/11/16/explicit-architecture-01-ddd-hexagonal-onion-clean-cqrs-how-i-put-it-all-together/) -- Herberto Graca (opinionated synthesis, not canonical source)
