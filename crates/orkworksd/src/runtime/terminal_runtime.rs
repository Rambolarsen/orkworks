...
#[cfg(test)]
pub(crate) fn record_terminal_input(state: &Arc<AppState>, id: &str, data: &str) -> Option<()> {
...
/// The metadata read/write/append tail below runs inside `tokio::task::spawn_blocking`,
/// matching the pattern already used for multi-step `MetadataStore` sequences elsewhere
/// in this crate (see `select_terminal_plan`/`report_session_plan_path` in
/// `http/session_handlers.rs`) — it's local `std::fs` I/O and must not block the calling
/// Tokio worker thread.
...
mod tests {
    ...
    fn pending_action_queue_replaces_resize_with_latest() {
        ...
        assert_eq!(
            queue.take_next(),
            Some(TerminalAction::Resize { rows: 30, cols: 40 })
        );
        assert_eq!(queue.take_next(), None);
    }
    ...
    fn pending_action_queue_drains_input_and_resize_before_kill_so_nothing_typed_is_lost() {
        ...
        assert_eq!(
            queue.take_next(),
            Some(TerminalAction::Resize { rows: 10, cols: 20 })
        );
