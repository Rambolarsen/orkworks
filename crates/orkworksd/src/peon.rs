...
pub(crate) struct PeonInferenceLease {
    id: String,
    epoch: u64,
}

impl PeonScheduler {
    pub(crate) fn state_for(&self, id: &str) -> PeonSchedulerState {
        self.states.get(id).copied().unwrap_or_default()
    }

    pub(crate) fn set_state(&mut self, id: &str, state: PeonSchedulerState) {
        self.states.insert(id.to_owned(), state);
    }

    pub(crate) fn claim_due_normal_inference(
        &mut self,
        id: &str,
        deadline: Instant,
    ) -> Option<(PeonInferenceLease, Option<String>)> {
        if matches!(self.state_for(id), PeonSchedulerState::IdleWaitingForUserInput | PeonSchedulerState::FinalScan | PeonSchedulerState::Inferring) {
            return None;
        }
        let due_output = self.last_output.get(id).is_some_and(|at| *at <= deadline);
        let pending_label = self.pending_labels.remove(id);
        if !due_output && !pending_label {
            return None;
        }
        let lease = PeonInferenceLease {
            id: id.to_owned(),
            epoch: *self.epochs.entry(id.to_owned()).or_insert(0),
        };
        self.states.insert(id.to_owned(), PeonSchedulerState::Inferring);
        Some((lease, self.label_hints.remove(id)))
    }

    pub(crate) fn due_normal_ids(&self, deadline: Instant) -> Vec<String> {
        let mut ids: HashSet<String> = self
            ...
            .map(|(id, _)| id.clone())
            .collect();
        ids.extend(self.pending_labels.iter().cloned());
        ids.into_iter().collect()
    }

    pub(crate) fn output_at(&self, id: &str) -> Option<Instant> {
        self.last_output.get(id).copied()
    }

    pub(crate) fn ensure_output_origin(&mut self, id: &str, now: Instant) {
        self.last_output.entry(id.to_owned()).or_insert(now);
    }

    pub(crate) fn lease_is_current(&self, lease: &PeonInferenceLease) -> bool {
        self.epochs.get(&lease.id).copied().unwrap_or_default() == lease.epoch
            && self.state_for(&lease.id) == PeonSchedulerState::Inferring
    }
    ...
    pub(crate) fn invalidate_for_ending(&mut self, id: &str) {
        *self.epochs.entry(id.to_owned()).or_insert(0) += 1;
        ...
        self.states.insert(id.to_owned(), PeonSchedulerState::WaitingForOutput);
    }

    pub(crate) fn request_observation_from_output(&mut self, id: impl AsRef<str>, now: Instant) -> bool {
        let id = id.as_ref();
    ...
    pub(crate) fn resume_after_qualifying_input(
        &mut self,
        id: &str,
        hint: Option<String>,
        now: Instant,
    ...
    pub(crate) fn hold_idle(&mut self, id: &str) {
        self.last_output.remove(id);
        self.label_hints.remove(id);
        self.pending_labels.remove(id);
        self.states.insert(id.to_owned(), PeonSchedulerState::IdleWaitingForUserInput);
    }

    pub(crate) fn complete_normal_inference(
        &mut self,
        lease: &PeonInferenceLease,
        persisted_idle: bool,
        ...
        if persisted_idle {
            self.hold_idle(&lease.id);
        } else if let Some(now) = retry_at {
            self.last_output.insert(lease.id.clone(), now);
    ...
    pub(crate) fn has_pending_normal_work(&self, id: &str) -> bool {
        self.last_output.contains_key(id) || self.pending_labels.contains(id)
    }

    pub(crate) fn has_pending_label(&self, id: &str) -> bool {
        self.pending_labels.contains(id)
    }

    pub(crate) fn clear_tracking(&mut self, id: &str) {
...
"#,
    ...
    fn scheduler_state_serializes_as_api_value() {
        assert_eq!(
            serde_json::to_string(&PeonSchedulerState::IdleWaitingForUserInput).unwrap(),
            "\"idle_waiting_for_user_input\"",
        );
    }

    #[test]
    fn idle_hold_clears_pending_observation_work() {
        let now = std::time::Instant::now();
        let mut scheduler = PeonScheduler::default();

        scheduler.request_observation_from_output("session", now);
        scheduler.resume_after_qualifying_input("session", Some("continue".into()), now);
        scheduler.hold_idle("session");

        assert_eq!(
            scheduler.state_for("session"),
            PeonSchedulerState::IdleWaitingForUserInput,
        );
        assert!(!scheduler.has_pending_normal_work("session"));
    }
}
