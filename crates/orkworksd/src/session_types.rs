...
mod tests {
    ...
    #[test]
    fn session_info_json_pins_public_key_shape_without_snapshot_bookkeeping_keys() {
        // Pins SessionInfo's static serde shape only (issue #394): the public
        // finalObservedStatus key exists and the internal snapshot bookkeeping
        // keys on SessionMetadata are not part of this struct's output. The
        // snapshot->value extraction behavior is pinned separately by
        // session_view's merge_live_session_info_extracts_only_the_snapshot_value.
