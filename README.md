...
- Session git-context fields (`repo_root`/`branch`/`dirty`/etc.) reflect each session's live PTY-process cwd, probed cross-platform (Linux/macOS/Windows) via the `sysinfo` crate, so a session that `cd`s or `git worktree add`s mid-session no longer shows its frozen launch-time location (see [ADR 0031](docs/adr/0031-live-session-cwd-via-sysinfo-probe.md))
