...
- Codex sessions capture their native session ID through the generated/local four-event hook bundle (`SessionStart`, `UserPromptSubmit`, `PermissionRequest`, and `Stop`), enabling exact resume and deterministic turn attention. `SessionStart` remains identity-only; the other three events report `working` or `waiting_for_input` only for the matching live session (see [ADR 0051](docs/adr/0051-codex-deterministic-attention-hooks.md))
