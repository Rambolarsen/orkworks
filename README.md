...
- Session `label` (title) carries `labelSource` provenance: Peon and authenticated Codex native names may refine automatic labels, while explicit user labels win; it remains decoupled from the turn-by-turn summary/checkpoint log (see [ADR 0062](docs/adr/0062-codex-native-session-labels.md))
