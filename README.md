...
- Claude Code's owned integration installs a synchronous `PostToolUse` `Write|Edit` hook whose shared reporter passes `--report-plan-path`, forwarding the hook payload's `tool_input.file_path` to `/plan-path` and skipping the generic attention + harness-session POSTs. `ToolHookContract::reports_plan_path` declares the opt-in so the framework stays open to non-Claude harnesses (see [ADR 0038](docs/adr/0038-claude-plan-path-post-tool-use-hook.md))
