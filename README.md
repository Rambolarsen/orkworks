...
- Raw terminal replay is bounded to the newest 1,000 lines and 1 MiB; dead sessions display that saved output read-only. The current-summary snapshot (`summary`/`summarySource`/`summaryConfidence`/`summaryObservedAt`) is the live session-situation record; durable workflow-friction evidence is recorded separately as `WorkflowObservation`s for Taskmaster (see [ADR 0042](docs/adr/0042-workflow-observations-replace-summary-checkpoints.md), superseding [ADR 0024](docs/adr/0024-bounded-terminal-replay-durable-summary-checkpoints.md))
...
- Session `label` (title) is a one-shot Peon-authored topic, decoupled from the turn-by-turn current-summary snapshot (see [ADR 0042](docs/adr/0042-workflow-observations-replace-summary-checkpoints.md), superseding [ADR 0029](docs/adr/0029-session-label-topic-vs-activity-summary.md))
- Peon and coding agents can record immutable `WorkflowObservation` evidence (repetition, obstacle, missing context, assumption, correction, workaround, or verification gap) through a shared recording module; Taskmaster correlates it into dismiss-only `improve_workflow` recommendations without ever auto-applying the suggested change (see [ADR 0042](docs/adr/0042-workflow-observations-replace-summary-checkpoints.md))
...
- `~/.orkworks/workspaces/<hash>/events/<id>.ndjson` — append-only event log; existing records may still carry the superseded summary-checkpoint fields from [ADR 0024](docs/adr/0024-bounded-terminal-replay-durable-summary-checkpoints.md) for read compatibility, but no new checkpoint is appended
...
- The current-summary snapshot (`summary`, `summarySource`, `summaryConfidence`, `summaryObservedAt`) is the selected session's live situation headline; the "Task history" checkpoint list and its `summary-log` route are removed. `label` (title) remains a stable, one-shot Peon-authored topic, decoupled from that turn-by-turn snapshot (see [ADR 0042](docs/adr/0042-workflow-observations-replace-summary-checkpoints.md))
- Taskmaster proposes cross-session transitions, including a passive, dismiss-only `improve_workflow` recommendation correlated from `WorkflowObservation` evidence; every v1 transition that starts, resumes, or focuses a session still requires explicit user approval (see [ADR 0042](docs/adr/0042-workflow-observations-replace-summary-checkpoints.md))
