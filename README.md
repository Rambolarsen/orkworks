...
- Codex sessions capture their native session ID through the generated/local four-event hook bundle (`SessionStart`, `UserPromptSubmit`, `PermissionRequest`, and `Stop`), enabling exact resume and deterministic turn attention. `SessionStart` remains identity-only; `UserPromptSubmit` reports `working`, `PermissionRequest` reports `waiting_for_input`, and `Stop` reports `idle` for the matching live session (see [ADR 0051](docs/adr/0051-codex-deterministic-attention-hooks.md))
