...
- Codex sessions capture their native session ID through the generated/local four-event hook bundle (`SessionStart`, `UserPromptSubmit`, `PermissionRequest`, and `Stop`), enabling exact resume and deterministic turn attention. The owning ID is protected from nested Codex processes, and resume requires the exact thread's saved local rollout; OrkWorks never falls back to the latest conversation. `SessionStart` remains identity-only; `UserPromptSubmit` reports `working`, `PermissionRequest` reports `waiting_for_input`, and `Stop` reports `idle` for the matching live session (see [ADR 0051](docs/adr/0051-codex-deterministic-attention-hooks.md) and [ADR 0066](docs/adr/0066-codex-exact-session-identity-and-resume.md))
