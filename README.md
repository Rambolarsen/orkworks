...
- Codex sessions capture their native session ID through the generated/local four-event hook bundle (`SessionStart`, `UserPromptSubmit`, `PermissionRequest`, and `Stop`), enabling exact resume and deterministic turn attention. Internal Codex CLI subagents remain under their parent OrkWorks session and get no separate OrkWorks ID. A recorded `/clear` can replace the native ID, and resume requires the exact thread's saved local rollout; OrkWorks never falls back to the latest conversation. `SessionStart` remains identity-only; `UserPromptSubmit` reports `working`, `PermissionRequest` reports `waiting_for_input`, and `Stop` reports `idle` for the matching live session (see [ADR 0051](docs/adr/0051-codex-deterministic-attention-hooks.md) and [ADR 0068](docs/adr/0068-codex-subagents-share-owning-session-identity.md))
...
- Session `label` (title) carries `labelSource` provenance: Peon and authenticated Codex native names may refine automatic labels, while explicit user labels win; it remains decoupled from the turn-by-turn summary/checkpoint log (see [ADR 0063](docs/adr/0063-codex-native-session-labels.md))
- Coding agents can record immutable `WorkflowObservation` evidence (repetition, obstacle, missing context, assumption, correction, workaround, or verification gap) through the authenticated `POST /sessions/:id/workflow-observations` route, gated by a per-session `ORKWORKS_REPORT_TOKEN` capability generated from OS randomness at session start/resume and never persisted, logged, or serialized. Peon-inferred recording and Taskmaster's dismiss-only `improve_workflow` correlation are also implemented; the separate current-summary migration remains pending (see [ADR 0042](docs/adr/0042-workflow-observations-replace-summary-checkpoints.md))
...
- Taskmaster consumes Peon reports and workspace context to propose the next session or user action; **Analyze now** can request another Brain-backed improvement after the active one is implemented or dismissed
- Taskmaster Fix with AI carries a stable recommendation ID into the active session; the agent can inspect the recommendation through the local sidecar and report verified completion through its session-scoped capability (see [ADR 0050](docs/adr/0050-taskmaster-recommendation-aware-agent-handoff.md))
...
- `~/.orkworks/workspaces/<hash>/.sidecar.lock` — retained lock file whose OS advisory lock identifies the sidecar currently owning workspace metadata (see [ADR 0052](docs/adr/0052-single-writer-workspace-lease.md))
...
- Peon reads terminal output and writes inferred metadata; Peon itself never types into terminals. The two explicit, user-confirmed exceptions that do are the session-plan review prompt ([ADR 0034](docs/adr/0034-user-approved-session-review-prompt.md)) and Taskmaster's `improve_workflow` "Fix with AI" accept action, which sends a generated fix prompt into the user's active session through the same mechanism ([ADR 0048](docs/adr/0048-improve-workflow-accept-sends-fix-prompt-to-active-session.md))
...
- `GET /sessions/:id/summary-log` returns checkpoints in append order as `{entries: [{timestamp, summary, source, confidence}]}`; `confidence` is nullable and missing data returns `{entries: []}`. Rendered in the session detail panel as "Task history" — distinct from the session's `label` (title), which carries provenance and is not this turn-by-turn activity log (see [ADR 0063](docs/adr/0063-codex-native-session-labels.md))
- Taskmaster proposes cross-session transitions; every v1 transition requires explicit user approval. (design, not yet implemented — see issue #313) A current-summary snapshot (`summary`, `summarySource`, `summaryConfidence`, `summaryObservedAt`) is planned to become the selected session's live situation headline, replacing the "Task history" checkpoint list and its `summary-log` route, alongside an `improve_workflow` Taskmaster recommendation correlated from `WorkflowObservation` evidence, which can be dismissed or explicitly accepted to send a generated fix prompt into the user's active session (see [ADR 0042](docs/adr/0042-workflow-observations-replace-summary-checkpoints.md), [ADR 0048](docs/adr/0048-improve-workflow-accept-sends-fix-prompt-to-active-session.md))
...
| [surfacing-blind-spots](skills/surfacing-blind-spots/SKILL.md) | Planning checkpoint, close-out, and quality-improvement audit workflow for investigated uncertainties and project blind spots |
| [babysitting-pull-requests](skills/babysitting-pull-requests/SKILL.md) | Keeps open PRs moving through complete comment, CI, and review-state inventories |
