...
- `~/.orkworks/workspaces/<hash>/workflow-observations/<session-id>.ndjson` and `.../sequence` — bounded (1,000 records/2 MiB per session), sequenced, immutable workflow-friction evidence recorded through one shared module from an authenticated agent-report route (`POST /sessions/:id/workflow-observations`); Peon-inferred recording remains design, not yet implemented — see issue #313. See [ADR 0042](docs/adr/0042-workflow-observations-replace-summary-checkpoints.md).
