...
- **Recorded size known:** construct the xterm `Terminal` with `cols`/`rows` fixed to the recorded values (no `FitAddon`/`ResizeObserver`-driven reflow — the buffer's wrap points are set once and never recomputed). Everything inside xterm's root `.xterm` element (`.xterm-viewport`, `.xterm-screen`, the canvases) is absolutely positioned, so `.xterm` has no natural intrinsic size of its own to measure or to size via CSS (`max-content` would just collapse it to empty) — instead xterm's own renderer sets an explicit inline pixel size (`style.width`/`style.height`, from `cols * cellWidth` / `rows * cellHeight`) on its internal `.xterm-screen` element on every render, which *is* a true measurement of the recorded grid's natural pixel size, unaffected by the container. On the first `onRender` event (public API — fires after xterm's initial paint), read `.xterm-screen`'s `getBoundingClientRect()` once and cache it as the natural size, then set that same width/height as an explicit inline pixel style directly on the root `.xterm` element (inline styles win over the shared `.terminal-container .xterm { width: 100%; height: 100%; }` stylesheet rule in `App.css` used by the live terminal — no CSS file changes needed, and the live-terminal path in `CenterPanel.tsx` is untouched) and apply `transform: scale(min(1, containerWidth / naturalWidth, containerHeight / naturalHeight))` with `transform-origin: top left` on it. `.terminal-container` already fills the panel via the existing `flex: 1` / `overflow: hidden` rules, so the transform just visually shrinks the now-explicitly-sized element inside the box that's already there, and `overflow: hidden` clips any remainder. A `ResizeObserver` on the container recomputes only the scale ratio against the cached natural size on resize — cheap, since it's a CSS transform, not a re-render.
