...
**Goal:** Give every unread live-session result a visible dot whose color communicates the latest normalized attention tone without changing unread-state behavior.

**Architecture:** Keep `sessionUnread.ts` as the sole derivation and latch for unread IDs. Change only `StatusIndicator`'s unread rendering and its CSS: unread always renders as a dot, and idle/working unread dots receive a blue override while existing needs-you, blocked, and failed colors continue to express their normalized tones.
...
- Use blue for unread `idle`, `stale`, `working`, `running`, and `creating` tones; amber for `blocked`, `checking_capacity`, and `capped`; red for `failed`.
- Keep the Electron main-process and renderer import boundary intact.
- Do not add a dependency.

---

### Task 1: Render unread working results as dots and apply the unread palette
...
- Produces: For every `variant="unread"`, a `status-indicator-unread` dot with `aria-label={\`Unread: ${label}\`}`. Its `data-attention` continues to carry the current normalized tone for CSS.
...
- [ ] **Step 1: Write the failing source-level regression assertions**
  ...
  working precedence with assertions that require all unread variants to use
  the dot branch and prohibit the working exception:
  ...
  Add CSS assertions that require the blue unread override for both idle and
  working without weakening the existing tone-selector coverage:
  ...
  read-state icon test and add a source assertion that the unread branch is
  not gated on `working`.
...
- [ ] **Step 2: Run the focused tests to verify the regression assertions fail**
  ...
  `tone !== "working"` and the idle/working unread CSS overrides do not exist.
...
- [ ] **Step 3: Make the minimal rendering and CSS change**
  ...
  unconditional so an unread working session renders the same accessible dot
  form as every other unread session:
  ...
  .status-indicator-unread[data-attention="working"] {
