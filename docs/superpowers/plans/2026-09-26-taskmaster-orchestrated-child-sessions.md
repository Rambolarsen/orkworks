...
- A child-scoped authenticated turn-completion receipt moves the task to `needs_parent_result` when the harness reports its turn complete/idle; the interactive child session remains open and manageable. If the harness has no reliable completion integration, the UI exposes an explicit user action to mark the turn ready for review.
- `POST /sessions/{parent_id}/orchestration/tasks/{task_id}/turn-completion` is the fallback when the child harness has no reliable completion integration. It requires `ORKWORKS_OPEN_PLAN_TOKEN`, the exact approved plan revision, launch reservation ID, child session ID, and expected task version; it is accepted only while that child is nonterminal and the task is `running`, then idempotently moves it to `needs_parent_result`.
...
- [ ] Test the Electron-authorized manual turn-completion action moves a live `running` child to `needs_parent_result`, checks revision/reservation/session/task-version identity, and rejects stale or mismatched requests.
