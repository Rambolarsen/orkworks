...
- `harness_registry.rs` — built-in harness configs (`HarnessConfig`: id, name, command/args, default model, model prefix, voice capabilities) and adapters, `resolve_adapter_harness_id`, `default_shell_command`, disk persistence helpers for `~/.orkworks/harnesses.json`. Note: a unification that folds peon inference parameters into `HarnessConfig` (a `HarnessPeonConfig` sub-struct, with `providers.rs` deriving `ProviderDefinition`s from it) is designed but **not yet implemented** — see `docs/superpowers/specs/2026-07-03-harness-registry-unification-design.md`. Today, adding a harness with peon support still requires a matching entry in `providers.rs`.
...
- `providers.rs` — fixed provider registry (`builtin_provider_registry()`), applied-settings store, persisted runtime state, fallback runner (`run_inference` skips disabled/capped providers in fallback order), model listing (`list_models` runs each provider's configured list-models CLI command). `update_session_capping` keeps provider capacity state in sync with per-session observations after each peon scan cycle, including a pending post-resume capacity-check set that surfaces as the `checking_capacity` effective state in `GET /providers`. On Unix, `ProcessRunner` calls `setsid()` + closes inherited fds ≥ 3 before spawning the harness subprocess (via `libc`), preventing PTY leakage into provider processes. This module still carries the historical `Provider*` names, but today it is modeling the Peon inference tool registry rather than the user-facing coding-tool selector. It exposes `GET /providers` for live runtime state, `GET /providers/:id/models` for available models, and `POST /settings/providers` for durable settings application. The session Peon loop routes through `ProviderManager::run_inference`. Per-provider peon model is configured in Settings.
