...
- `POST /taskmaster/recommendations/:id/accept` — implemented (ADR 0048). Takes a caller-supplied `sessionId` (the desktop's current `activeSessionId`, never the backend's best-effort-persisted last-active value) and an optional `prompt` override, and sends a generated fix prompt into that session's live PTY via `terminal_runtime::submit_approved_input` — the same path `request_plan_review` uses. No session is created, resumed, or reconfigured; on success `status` transitions `proposed` → `accepted` and `targetSessionId` is set to the session the prompt was sent to.
...
- `http/` — HTTP handler submodules (`ErrorResponse` declared in `http/mod.rs`):
  ...
  - `taskmaster_handlers.rs` — recommendation list/detail/dismiss/accept adapters. `accept` (ADR 0048) sends a generated fix prompt into a caller-specified, already-live session; it starts no session of its own.
