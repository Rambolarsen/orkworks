...
- `harness.rs` — neutral command and resume types plus `definition`, `registry`, and `store` submodules. The versioned built-in resource and user overrides resolve once into an immutable `ResolvedHarnessRegistry`; each runtime operation captures one snapshot. Definitions declare launch, resume, model, Peon, capacity, signal, integration, and voice capabilities without adapter code paths.
