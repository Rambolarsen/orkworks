...
- [ ] A manual Brain analysis bypasses only the per-workspace cooldown, consumes the daily allowance, and is refused while an `improve_workflow` recommendation is proposed, accepted, or executing; the user is directed to the existing Fix with AI handoff.
