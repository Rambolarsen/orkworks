...
- **Codex identity and resume integrity**: retain the first accepted native Codex session ID; a different ID may replace it only on an authenticated `SessionStart` with `source=clear` after OrkWorks recorded the explicit reset, and only when the hook identifies the same live Codex process that owns the session. If process ownership cannot be established, keep the existing ID. Resume only with that exact ID and only when the corresponding thread row and rollout file exist in the supported local Codex store. Missing or unsaved IDs never fall back to another Codex conversation. See [ADR 0066](../docs/adr/0066-codex-exact-session-identity-and-resume.md).
